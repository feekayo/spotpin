...
var app = express();
var url = require('url');
var user_galleries = require('modules/user_galleries');



router.get('/user_galleries/:id', function(request, response){
	response.send(JSON.stringify(user_galleries.query(request.params.id)))
});
...
router.get('/', function(request, response) {
	...
	if(Object.keys(get_params).length == 2){
		...
		if(arg!=''&&value!=''&&arg!=null&&value!=null){
			response.send(JSON.stringify(user_galleries.query_by_arg(arg, value)));
		}else{
		...
		response.send(JSON.stringify(user_galleries.list()));
	}else{
		response.send('Hmmmm... something\'s not right, Are you sure you specified the URI in the right format?')
	}
