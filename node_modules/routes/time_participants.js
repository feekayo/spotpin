...
var app = express();
var url = require('url');
var time_participants = require('modules/time_participants');



router.get('/time_participants/:id', function(request, response){
	response.send(JSON.stringify(time_participants.query(request.params.id)))
});
...
router.get('/', function(request, response) {
	...
	if(Object.keys(get_params).length == 2){
		...
		if(arg!=''&&value!=''&&arg!=null&&value!=null){
			response.send(JSON.stringify(time_participants.query_by_arg(arg, value)));
		}else{
		...
		response.send(JSON.stringify(time_participants.list()));
	}else{
		response.send('Hmmmm... something\'s not right, Are you sure you specified the URI in the right format?')
	}
