...
var app = express();
var url = require('url');
var likes = require('modules/likes');



router.get('/likes/:id', function(request, response){
	response.send(JSON.stringify(likes.query(request.params.id)))
});
...
router.get('/', function(request, response) {
	...
	if(Object.keys(get_params).length == 2){
		...
		if(arg!=''&&value!=''&&arg!=null&&value!=null){
			response.send(JSON.stringify(likes.query_by_arg(arg, value)));
		}else{
		...
		response.send(JSON.stringify(likes.list()));
	}else{
		response.send('Hmmmm... something\'s not right, Are you sure you specified the URI in the right format?')
	}
