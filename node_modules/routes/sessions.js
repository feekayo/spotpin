...
router.get('/sessions/:id', function(request, response){
	response.send(JSON.stringify(sessions.query(request.params.id)))
});
...
router.get('/', function(request, response) {
	...
	if(Object.keys(get_params).length == 2){
		...
		if(arg!=''&&value!=''&&arg!=null&&value!=null){
			response.send(JSON.stringify(sessions.query_by_arg(arg, value)));
		}else{
		...
		response.send(JSON.stringify(sessions.list()));
	}else{
		response.send('Hmmmm... something\'s not right, Are you sure you specified the URI in the right format?')
	}
